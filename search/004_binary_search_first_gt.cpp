...
public:
    BinarySearch()
    {}
    int search1stGtNum(const std::vector<int>& datas,
               const int dest);
private:
...
int BinarySearch::search1stGtNum(const std::vector<int>& datas,
                         const int dest)
{
...
int main()
{
    ...
    int ret = -1;
    {
        ...
        int dest = 3;
        ret = binarySearch.search1stGtNum(datas, dest);
    ...
    }
    {
        ...
        int dest = 5;
        ret = binarySearch.search1stGtNum(datas, dest);
        LOG_IF(INFO, ret == 5) << "Case 3 Pass";
        CHECK(ret == 5) << "Case 3 Failed, Dest Not Found:" << dest << " ret:" << ret;
    }
    {
    ...
    }
    {
        ...
        int dest = 6;
        ret = binarySearch.search1stGtNum(datas, dest);
        LOG_IF(INFO, ret == 6) << "Case 5 Pass";
        CHECK(ret == 6) << "Case 5 Failed, Dest Not Found:" << dest;
    }
    {
        ...
        int dest = 6;
        ret = binarySearch.search1stGtNum(datas, dest);
        LOG_IF(INFO, ret == -1) << "Case 6 Pass";
        CHECK(ret == -1) << "Case 6 Failed, Dest Not Found:" << dest << " ret:" << ret;
    }
    return 0;
}
